...
- **LM/LO Relief Request Navigation Update**: Updated navigation workflow for Logistics Managers and Logistics Officers. Sidebar menu "Create Request (On Behalf of Agency)" now navigates to the Relief Requests list page (showing all requests). Page title renamed to "Relief Requests on Behalf of Agency" for LM/LO only; agency users still see "Relief Requests". "New Request" button on list page routes to the on-behalf form (`packaging.create_request_on_behalf`) for LM/LO, standard form for agencies. Maintains distinct user experiences while consolidating navigation.
