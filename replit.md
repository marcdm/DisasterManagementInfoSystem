...
- **Batch Allocation DTO Pattern**: Logistics Manager drawer uses BatchDTO (NamedTuple) to safely carry released availability through all service layers. The DTO encapsulates batch, released_available, and is_allocated fields, eliminating reliance on ORM attribute attachment. Implementation includes Set A (allocated batches always shown) and Set B (available batches with released_available > 0) logic, per-warehouse FEFO/FIFO sorting, and smart truncation based on requested quantity. Temporary _released_available attachment maintains backward compatibility with existing helper functions.
