...
- **Batch Allocation DTO Pattern**: Logistics Manager drawer uses BatchDTO (NamedTuple) to safely carry released availability through all service layers. The DTO encapsulates batch, released_available, and is_allocated fields. Implementation includes Set A (allocated batches always shown) and Set B (available batches with released_available > 0) logic, per-warehouse FEFO/FIFO sorting, and smart truncation based on requested quantity. The DTO pattern ensures released_available is calculated once and preserved throughout the allocation pipeline. For backward compatibility with existing helper functions (assign_priority_groups, sort_batches_for_drawer), _released_available is temporarily reattached when interfacing with these legacy functions.
