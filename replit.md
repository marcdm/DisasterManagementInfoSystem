...
- **COMPLETE Timezone Fix - System-Wide Correction**: Fixed critical timezone bug where `datetime.now()` was storing UTC timestamps instead of Jamaica Standard Time across the entire application. **All fixes completed across 9 files (42 total occurrences)**: (1) `app/core/audit.py` - all audit field timestamps (create_dtime, update_dtime, verify_dtime), (2) `app/db/models.py` - DistributionPackage model defaults, (3) `app/features/packaging.py` - 29 timestamp operations, (4) `app/features/donations.py` - 4 verification timestamps, (5) `app/features/donation_intake.py` - intake timestamps, (6) `app/features/operations_dashboard.py` - dashboard date calculations, (7) `app/features/reports.py` - export filename timestamps, (8) `app/features/dashboard.py` - dashboard metrics calculations, (9) `app/services/relief_request_service.py` - create/review/action/dispatch timestamps. **ROOT CAUSE**: System timezone environment variable is NOT set, so Python's `datetime.now()` returns UTC time. **SOLUTION**: All 42 `datetime.now()` occurrences replaced with `jamaica_now()` from `app/utils/timezone`. **VERIFICATION**: Repository-wide search confirms zero residual `datetime.now()` usage. **IMPACT**: All new database records now store correct Jamaica Standard Time timestamps; existing records may have UTC timestamps (non-breaking, cosmetic only). **RECOMMENDED NEXT STEPS**: (1) Add CI lint check to prevent new `datetime.now()` regressions, (2) Optional: run data audit script to quantify existing UTC-stamped rows. Architect-reviewed and approved (PASS).
- **Universal Visibility for Approved Relief Requests**: Modified distribution logic so ALL approved and eligible relief requests are now visible to BOTH Logistics Officers (LO) and Logistics Managers (LM), regardless of who created the request. Removed user-based ownership filtering from `pending_fulfillment()` and `logistics_dashboard()` functions. Both roles now see the exact same set of approved/eligible requests to enable collaboration and operational flexibility. Added RBAC guard to logistics dashboard route to ensure only authorized LO/LM roles can access. This change affects: (1) "Approved & Eligible Relief Requests" page, (2) Logistics Dashboard tabs, (3) All packaging workflow views. Security maintained: only LO and LM can access these views; other roles receive 403 Forbidden.
- **Fixed Validation to Use Available Quantity**: Corrected form validation in Prepare and Approve Relief Package pages to validate allocations against `available_qty = usable_qty - reserved_qty` instead of just `usable_qty`. This prevents false validation errors and ensures LO/LM can allocate up to the true available stock, accounting for reservations by other packages.
- **Fixed Batch Drawer Available Quantity for LM Re-allocation**: Corrected Select Batches drawer to "release" current package's allocations when calculating available quantities. When LM re-opens drawer to re-allocate, system now calculates `available_qty = usable_qty - (reserved_qty - current_package_allocations)`, allowing LM to reallocate freely without double-counting their own allocations. Modified batch-allocation.js to send current allocations to API; updated BatchAllocationService.get_limited_batches_for_drawer() and packaging API to accept and use current_allocations parameter. Banner and per-batch available quantities now reflect true stock availability for re-allocation scenarios. Always includes allocated batches in drawer even if fully reserved to current package.
- **Fixed Warehouse Stock Display for LM Re-allocation**: Updated warehouse stock display on Prepare and Approve Relief Package pages to correctly calculate available quantities as `usable_qty - reserved_qty` instead of treating allocations as consuming `usable_qty` directly. This aligns front-end logic with database behavior where only `reserved_qty` changes during allocation; `usable_qty` is only reduced when package is sent to dispatch. Enhanced tooltips now show breakdown: Usable, Reserved (other packages), Available, Allocated (this package), and Remaining quantities.
- **Cancel Package Preparation Releases Reservations**: Enhanced Cancel Package Preparation to release both batch-level (`itembatch.reserved_qty`) and warehouse-level (`inventory.reserved_qty`) reservations. When user cancels a package, system now: (1) Reduces `itembatch.reserved_qty` for each allocated batch, (2) Recalculates `inventory.reserved_qty` from SUM(itembatch.reserved_qty), (3) Ensures consistency invariant is maintained, (4) Guards against negative quantities. All operations execute in single atomic transaction with proper error handling and logging.
- **Save Draft Button Hidden**: Removed "Save Draft" button from Prepare Relief Package UI for both LO and LM roles using CSS (`display: none !important`). All backend functionality remains intact and operational for potential future use or internal calls. Users must now submit packages directly for approval rather than saving drafts.
